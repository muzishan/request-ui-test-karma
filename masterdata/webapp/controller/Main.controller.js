    ...
    return Controller.extend("com.kion.sdr.ui.masterdata.controller.Main", {
        ...
        setInitialFilterForSalesOrgCombox: function () {
            ...
            if (this.currentUser.salesOfficeBranches.length > 0) {
                ...
                this.byId("salesOrgCustomerSelect").getBinding("items").filter(new Filter({ filters: filters, and: false }));
            }
        ...
        getNewEntityData: function (sEntityType) {
            switch (sEntityType) {
                ...
                case "batteryCharger":
                    return { category: 'BATTERY', code: '', brand: 'LMH', description: '', active: true };
        ...
        liveCheckFormValidation: function (oEvent) {
            ...
            if (sEntityType === 'batteryChargerDiscount') {
                if (oEvent.getSource().getProperty('name') === 'batteryChargerCode') {
                    const brand = oEvent.getSource().getSelectedItem().getText().split(' - ')[1];
                    this.setSalesOrgComboBoxFilterForBatteryChargerDiscount(brand);
                    this.setSeriesComboBoxFilterForBatteryChargerDiscount(brand);
                }
            }
        },
        setSalesOrgComboBoxFilterForBatteryChargerDiscount: function (brand) {
            this.getView().byId("comboBoxSalesOrg").getBinding('items').filter(
                [
                    new Filter({
                        path: 'brand',
                        operator: FilterOperator.EQ,
            ...
            if (this.getView().byId("comboBoxSalesOrg").getSelectedItem() && !this.getView().byId("comboBoxSalesOrg").getSelectedItem().getText().includes(brand)) {
                this.getView().byId("comboBoxSalesOrg").clearSelection();
            }
        },
        setSeriesComboBoxFilterForBatteryChargerDiscount: function (brand) {
            this.getView().byId("comboBoxSeries").getBinding('items').filter(
                [
                    new Filter({
                        path: 'brand',
                        operator: FilterOperator.EQ,
            ...
            if (this.getView().byId("comboBoxSeries").getSelectedItem() && !this.getView().byId("comboBoxSeries").getSelectedItem().getText().includes(brand)) {
                this.getView().byId("comboBoxSeries").clearSelection();
            }
        },
        ...
        onPressEditButton: function (oEvent) {
            ...
            this.openEntityDialog(sEntityType);
            if (sEntityType === 'batteryChargerDiscount') {
                this.setSalesOrgComboBoxFilterForBatteryChargerDiscount(oContext.getObject().batteryCharger.brand);
                this.setSeriesComboBoxFilterForBatteryChargerDiscount(oContext.getObject().batteryCharger.brand);
            }
        },
        ...
        batteryChargerFilters: function () {
            ...
            const sBrand = this.byId("brandSelect").getSelectedKey();
            if (sCategory) {
        ...
        batteryChargerDiscountFilters: function () {
            ...
            const sBrand = this.byId("brandDiscountSelect").getSelectedKey();
            ...
            if (sBrand) {
                aFilters.push(new Filter({
                    path: "batteryCharger/brand",
                    operator: FilterOperator.EQ,
                    value1: sBrand
                }));
                this.filterStrForExportBatteryChargerDiscount.push(`contains(tolower(batteryCharger/brand),tolower('${sBrand}'))`);
            }
            if (sSalesOrgCodes.length > 0) {
                const filterStrArr = [];
                const filters = sSalesOrgCodes.map(function (code) {
                    filterStrArr.push(`salesOrg/code eq '${code}'`);
                    return new Filter({ path: 'salesOrg/code', operator: FilterOperator.EQ, value1: code, and: false });
                });
                aFilters.push(new Filter({ filters: filters, and: false }));
        ...
        customerFilters: function (oEvent) {
            ...
            if (sSalesOrgs.length > 0) {
                ...
                const filters = sSalesOrgs.map(function (ID) {
                    ...
                    return new Filter({ path: 'salesOrg_ID', operator: FilterOperator.EQ, value1: ID, and: false });
                });
                aFilters.push(new Filter({ filters: filters, and: false }));
            ...
            if (sIndustryCodes.length > 0) {
                ...
                const filters = sIndustryCodes.map(function (ID) {
                    ...
                    return new Filter({ path: 'industryCode_ID', operator: FilterOperator.EQ, value1: ID, and: false });
                });
                aFilters.push(new Filter({ filters: filters, and: false }));
                this.filterStrForExportCustomer.push(`(${filterStrArr.join(' or ')})`);
            }
            ...
            if (sCountrys.length > 0) {
                ...
                const filters = sCountrys.map(function (code) {
                    ...
                    return new Filter({ path: 'country_code', operator: FilterOperator.EQ, value1: code, and: false });
                });
                aFilters.push(new Filter({ filters: filters, and: false }));
                this.filterStrForExportCustomer.push(`(${filterStrArr.join(' or ')})`);
            }
        ...
        onClearFilters: function () {
            ...
            this.getView().byId("brandSelect").setSelectedKey("");
            ...
            this.getView().byId("brandDiscountSelect").setSelectedKey("");
        ...
        onRadioButtonSelect: function (oEvent) {
            ...
            } else if (sGroupId.includes("brandBatteryChargerDialogCheckbox")) {
                oModel = this.getView().getModel("batteryChargerCreateModel");
                sPropertyPath = "/brand";
        ...
        createBatteryChargerDiscountColumnConfig: function () {
            ...
            {
                label: this.getView().getModel("i18n").getResourceBundle().getText("brand"),
                property: "batteryCharger/brand",
                width: "20",
                type: EdmType.String
            },
