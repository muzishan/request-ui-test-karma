...
sap.ui.define([
	...
	QUnit.test("Should return update title when isEdit is true", function (assert) {
		const payload = {isEdit: true, productCode: 'productCode'};
		const sAddTitle = "Add";
		const sUpdateTitle = "Update";

		const result = Formatter.formatDialogTitle(payload, sAddTitle, sUpdateTitle);

		assert.strictEqual(result, "Update productCode", "The result should be 'Update' when isEdit is true")
	})
	...
	QUnit.test("Should return Add title when isEdit is false", function (assert) {
		...
		const result = Formatter.formatDialogTitle(payload, sAddTitle, sUpdateText);
